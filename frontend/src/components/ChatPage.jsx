...
// Helper to read whether tools are allowed or only limitations should be explained
const getAllowToolsParam = () => {
  if (typeof window !== "undefined") {
    const params = new URLSearchParams(window.location.search);
...
export default function ChatPage() {
  ...
  */
  ...
  // The system prompt dynamically reads the current dashboard state and URL tool configuration.
  const system = useMemo(() => {
    const noToolsInstruction = `If NO:
      - STRICT RESTRICTION: You MUST NOT call any tools to generate alternative explanation charts. Do NOT generate new figures.
      - ONLY explain why the currently displayed explanation (${visibleTexts.ui}) cannot answer the user's question based on its generation method, underlying assumptions, and boundaries.`;

    const allowToolsInstruction = `
      If NO:
    ...
    return `You are a helpful assistant explaining a machine learning model used as an automated tool to approve or decline credit limit increase applications. The user is viewing the applicant identified as ${applicantReference} in the dataset. Assume the user has no prior knowledge of AI. When answering questions, assume the user is asking about this applicant's specific profile unless stated otherwise. The model predicts each applicant a credit score from 0 to 100. A score of 50 or higher means the application is approved.
      ...
      ${allowTools ? allowToolsInstruction : noToolsInstruction}

      Guidelines:
      - Be concise. Use layperson-friendly language. MUST answer in less than 150 words.
      ${!allowTools ? "- Do not call any tools if the explanation cannot answer users' question. The intention is to examine whether this explanation can answer question or not." : ""}
  ...
  }, [userInstanceId, applicantReference, visibleTexts.system, visibleTexts.ui, allowTools])
  ...
  const handleDesignCQuestion = (q, category) => {
    ...
    if (category === "tellsYou") {
      ...
      promptText = allowTools
        ? `Explain briefly why the explanation "${visibleTexts.ui}" cannot answer my question: "${q}". Use the appropriate tool to generate the explanation that can answer it.`
        : `Explain briefly why the explanation "${visibleTexts.ui}" cannot answer my question: "${q}".`
    }
